...
def load(ex):
    ...
    if not p.exists():
        p=Path(f"data/raw_1m/{ex}/DRIFT_USDT_USDT/candles.parquet")
...
for i,zi in enumerate(z):
    ...
    if open_pos is None:
        ...
        if abs(zi)<0.3 or abs(zi)>1.5*ENTRY or hold>=15:
            ...
            net=gross-0.28
            trades.append((open_pos["i"], i, open_pos["z"], zi, gross, net, hold))
            open_pos=None
            if len(trades)>=1: break
print(trades)
if trades:
    i_in,i_out,zin,zout,gross,net,hold=trades[0]
    print(f"trade {i_in}->{i_out} z {zin:.2f}->{zout:.2f} gross {gross:.3f} net {net:.3f} hold {hold}m")
    ...
    html=f"""<html><head><meta charset=utf-8><title>DRIFT trade</title><style>body{{background:#111;color:#ddd;font-family:monospace;padding:16px}}h2{{color:#7cc4ff}}svg{{background:#1a1a1a;border:1px solid #333}}</style></head><body><h2>DRIFT bitget/mexc z=2.7 -- net {net:.3f}% gross {gross:.3f}%</h2><p>Entry bar {i_in} z={zin:.2f} ratio={ratio[i_in]:.4f} | Exit {i_out} z={zout:.2f} hold {hold}m | 1m real data, EMA50 shift 1, fee 0.28%</p><svg width="{W}" height="{H}"><polyline fill="none" stroke="#4a90e2" stroke-width="1.2" points="{pts_ratio}"/><polyline fill="none" stroke="#f5a623" stroke-width="1" points="{pts_ma}"/><line x1="{xe_in}" y1="20" x2="{xe_in}" y2="{H-30}" stroke="#ef5350" stroke-dasharray="4,4"/><line x1="{xe_out}" y1="20" x2="{xe_out}" y2="{H-30}" stroke="#26a69a" stroke-dasharray="4,4"/></svg><svg width="{W}" height="200"><polyline fill="none" stroke="#b0e0e6" stroke-width="1.2" points="{pts_z}"/><line x1="40" y1="{y_z(2.7):.1f}" x2="{W-20}" y2="{y_z(2.7):.1f}" stroke="#ef5350" opacity="0.6"/><line x1="40" y1="{y_z(-2.7):.1f}" x2="{W-20}" y2="{y_z(-2.7):.1f}" stroke="#ef5350" opacity="0.6"/><line x1="40" y1="{y_z(0.3):.1f}" x2="{W-20}" y2="{y_z(0.3):.1f}" stroke="#26a69a" opacity="0.6"/><line x1="{xe_in}" y1="10" x2="{xe_in}" y2="190" stroke="#ef5350" stroke-dasharray="4,4"/><line x1="{xe_out}" y1="10" x2="{xe_out}" y2="190" stroke="#26a69a" stroke-dasharray="4,4"/></svg><p style="color:#888;font-size:12px">Calculations on objective 1m close, EMA50 shift 1, no lookahead. Daily screener walk-forward: 3d window, top20 tradable.</p></body></html>"""
